...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // 设置黄色内容过滤器开关初始状态（强制开启，隐藏开关）
    const yellowFilterToggle = document.getElementById('yellowFilterToggle');
    if (yellowFilterToggle) {
        yellowFilterToggle.checked = true;
        yellowFilterToggle.disabled = true;
        localStorage.setItem('yellowFilterEnabled', 'true');
        const filterContainer = yellowFilterToggle.closest('.flex.items-center.justify-between.mb-3.pb-3.border-b');
...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `SlakeyTV - 免费在线视频搜索与观看平台`;
    } catch (e) {
...
async function search() {
    ...
    try {
        ...
        try {
            ...
            // 更新页面标题
            document.title = `搜索: ${query} - SlakeyTV`;
        } catch (e) {
            console.error('更新浏览器历史失败:', e);
            // 如果更新URL失败，继续执行搜索
        }
        ...
        const safeResults = allResults.map(item => {
            ...
            const sourceInfo = item.source_name ?
                `<span class="search-card-source-badge">${item.source_name}</span>` : '';
                ...
                <div class="search-result-card" 
                    ...
                    <div class="search-result-card-inner">
                        ...
                        <div class="search-card-body">
                            <div class="search-card-body-top">
                                <h3 class="search-card-title" title="${safeName}">${safeName}</h3>
                                
                                <div class="search-card-tags ${hasCover ? '' : 'justify-center'}">
                                    ${(item.type_name || '').toString().replace(/</g, '&lt;') ?
                    `<span class="search-card-tag search-card-tag-blue">
                                          ${(item.type_name || '').toString().replace(/</g, '&lt;')}
                                      </span>` : ''}
                                    ${(item.vod_year || '') ?
                    `<span class="search-card-tag search-card-tag-purple">
                                          ${item.vod_year}
                                      </span>` : ''}
                                </div>
                                <p class="search-card-remarks ${hasCover ? '' : 'text-center'}">
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'SlakeyTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'SlakeyTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'SlakeyTV-Settings_' + times + '.json');
}
